...
# in part 1.
file = open("T_DAILY_AVG_sorted.txt", 'r')
...
while True:
	try:
		Values_to_Input = input().strip()
		#Note that the strip command reads
        # in the data with removing the given characters that are extra and not
        # necessary with reading the data.  NOTE THIS DOES NOT REMOVE THE MISSING
        # VALUES, we will ignore the -9999.0 first value in the NEXT STEP.
	except EOFError:
		break
		# Our code will be read in until there is a break or no more lines
        # in the text file with the End of File (or EOFError command).
	...
	Values_to_Input = float(Values_to_Input)
	...
	# are included.
	StreamNumbers.append(Values_to_Input)
...
Minimum_T_DAILY_AVG = StreamNumbers[0]
...
# of [1].
Maximum_T_DAILY_AVG = StreamNumbers[-1]
...
Average_T_DAILY_AVG = sum(StreamNumbers)/len(StreamNumbers)
...
middleNumber = len(StreamNumbers) // 2
...
Median_T_DAILY_AVG = (StreamNumbers[middleNumber] + StreamNumbers[~middleNumber]) / 2
...
# Print the results
#print('min: %g, max: %g, average: %g, median: %g' % (Minimum_T_DAILY_AVG, Maximum_T_DAILY_AVG, Average_T_DAILY_AVG, Median_T_DAILY_AVG))
