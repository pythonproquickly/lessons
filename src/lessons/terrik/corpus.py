...
class Corpus:
    ...
    def fetch_url(self, url):
        ...
        if file_name is None:
            ...
            url_data = {
                "url": url,
                "content": data_dict[b"raw_content"][b"value"]
                if b"raw_content" in data_dict and b"value" in data_dict[b"raw_content"]
                else "",
                "http_code": int(data_dict[b"http_code"][b"value"]),
                "content_type": get_content_type(data_dict),
                "size": os.stat(file_name).st_size,
                "is_redirected": data_dict[b"is_redirected"][b"value"]
                if b"is_redirected" in data_dict
                and b"value" in data_dict[b"is_redirected"]
                else False,
                "final_url": data_dict[b"final_url"][b"value"]
                if b"final_url" in data_dict and b"value" in data_dict[b"final_url"]
                else None,
            }
