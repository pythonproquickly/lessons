...
def write_test_file(
    dir_path: pathlib.Path, sub_path: pathlib.Path, lines: [str]
) -> None:
...
TEST_FILES = [
    (pathlib.Path("test1.txt"), ["This is a line of text", "and this is another"]),
    (
    ...
    (pathlib.Path("Sub/test1.txt"), ["Hello, my name is Boo", "How are you today?"]),
    (
        pathlib.Path("Sub/youu.txt"),
        ["Or maybe it should be about you", "I cannot decide"],
    ),
    (
        pathlib.Path("Zzz/zzz.py"),
        ["print('Sleep...')", "for i in range(10):", "    print('ZZZZZZZZZZ')"],
    ),
]
...
def make_test_lines(test_directory_path: pathlib.Path) -> ["TestLine"]:
    ...
    test_lines.append(TestInputLine("R {}".format(str(test_directory_path))))

    test_lines.append(
        TestOutputLine(str(test_directory_path / pathlib.Path("test1.txt")), 10.0)
    )

    test_lines.append(
        TestOutputLine(str(test_directory_path / pathlib.Path("test2.txt")), 10.0)
    )

    test_lines.append(
        TestOutputLine(str(test_directory_path / pathlib.Path("Sub/meee.txt")), 10.0)
    )

    test_lines.append(
        TestOutputLine(str(test_directory_path / pathlib.Path("Sub/test1.txt")), 10.0)
    )

    test_lines.append(
        TestOutputLine(str(test_directory_path / pathlib.Path("Sub/youu.txt")), 10.0)
    )

    test_lines.append(
        TestOutputLine(str(test_directory_path / pathlib.Path("Zzz/zzz.py")), 10.0)
    )
...
def run_test() -> None:
    ...
    try:
        ...
        print_labeled_output(
            "FAILED", "The sanity checker has failed, for the reasons described above."
        )
...
def start_process() -> TextProcess:
    filenames_in_dir = [
        p.name for p in list(pathlib.Path.cwd().iterdir()) if p.is_file()
    ]
