...
logging.basicConfig(filename = 'AutoMPGData.log', level = logging.DEBUG, filemode = 'w')
    ...
    # By default, this class is INITIALLY HASHABLE!
        ...
        def __eq__(self, anotherObject):
            if type(self) == type(anotherObject):
                ...
                return NotImplemented # important to return NotImplemented instead of false.  Much easier for the Python console to run.
...
def sort_by_year(self):
    ...
    self.data.sort(key=lambda x: (x.year, x.make, x. model, x.mpg)) # We are using tuple comparison
...
def sort_by_mpg(self):
        # key takes a function with lambda function for the key.  return order of values sorted.
        self.data.sort(key=lambda y: (y.mpg, y.make, y. model, y.year))# We are using tuple comparison
        # so need to also include make and model attributes in our lambda key value
