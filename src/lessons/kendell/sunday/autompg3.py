...
'''
'''
LOGGING EXAMPLE FROM CLASS! :
import logging

logger = logging.getLogger()
logger.setLevel(logging.DEBUG) # makes sure debug messages are at least being processed.
...
fh = logging.FileHandler('logfile_ex.log', 'w')
...
class AutoMPGData:
        ...
        if os.path.exists("auto-mpg.clean.txt") == False:
            ...
            with open("auto-mpg.clean.txt") as cleanFile:
                ...
                for row in AutoReader:
                    ...
                    AutoMPGData.data.append([splitName[0], " ".join(
    ...
    def clean_data(self):
        if self.make == ("chevroelt" or "chevey"):
            # Notice the correct spelling is it is changed too.
            print(self.make == "chevrolet")
        ...
        elif self.make == ("vokswagen" or "vw"):
    ...
    def mpg_by_year(self, sort_by_mpg, sort_by_year):
            # following examples of sort definitions above.
            # We are using tuple comparison
            self.data.sort(key=lambda x: (x.year, x.mpg, x. model, x.make))
            # Notice order of sort is by year then mpg.
            self.data.sort(key=self.data.year.mpg)

    def mpg_by_make(self, sort_by_mpg, sort_by_year):
            # We are using tuple comparison
    ...
    # called by __load_data__
        ...
        '''
        Note the current output of the AutoMPGData.txt file in the same working directory is:
        DEBUG:root:very detailed information
    INFO:root:tracing info
    WARNING:root:something bad might happen
    ERROR:root:something bad happened
    CRITICAL:root:something really bad happened
    DEBUG:__main__:odd number encountered: 1
    INFO:__main__:odd number encountered: 1
    DEBUG:__main__:odd number encountered: 3
    INFO:__main__:odd number encountered: 3
    DEBUG:__main__:odd number encountered: 5
    INFO:__main__:odd number encountered: 5
    DEBUG:__main__:odd number encountered: 7
    INFO:__main__:odd number encountered: 7
    DEBUG:__main__:odd number encountered: 9
    INFO:__main__:odd number encountered: 9
        '''
    ...
    # The resulting output is in that file (when it is opened)!:
    '''
    DEBUG:root:very detailed information
    INFO:root:tracing info
    WARNING:root:something bad might happen
...
if args.data.sort == 'year':
    data.sort_by_year()
if args.data.sort == 'mpg':
    data.sort_by_mpg()
if args.data.sort == 'default':
